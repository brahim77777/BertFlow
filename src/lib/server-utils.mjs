...
export function toComponentName(name = "Component") {
  const cleaned = name
    .replace(/[^a-zA-Z0-9]+/g, " ")
    .trim()
    .split(/\s+/)
...
export function renderGeneratedComponent(component) {
...
export async function readRequestBody(request, maxLength = 1_000_000) {
  const chunks = [];
  let length = 0;
...
export function parseMultipartFile(body, contentType = "") {
  ...
  if (nextBoundary === -1) throw new Error("Multipart file boundary not found");
